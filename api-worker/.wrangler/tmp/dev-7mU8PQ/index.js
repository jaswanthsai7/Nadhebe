...
// .wrangler/tmp/bundle-hRJn68/checked-fetch.js
...
// .wrangler/tmp/bundle-hRJn68/strip-cf-connecting-ip-header.js
...
var CreateJobSchema = external_exports.object({
  ...
  sourceType: external_exports.enum(["youtube", "github", "website", "raw"]),
  multi: external_exports.boolean().optional()
});

// src/domain/issue.ts
var SubmitReviewSchema = external_exports.object({
  title: external_exports.string().min(3),
  slug: external_exports.string().min(3),
  ...
  author: external_exports.string().min(2),
  sourceUrl: external_exports.string().url(),
  markdown: external_exports.string().min(50),
  computedMetrics: external_exports.object({
    wordCount: external_exports.number().int().nonnegative(),
    readingTime: external_exports.number().int().nonnegative(),
    headingStructureOk: external_exports.boolean()
  }),
  aiInsights: external_exports.object({
    confidence: external_exports.number().min(0).max(1),
    factCheckSummary: external_exports.string()
  }),
  bundle: external_exports.object({
    newsletter: external_exports.string().optional(),
    twitterThread: external_exports.array(external_exports.string()).optional(),
    linkedInPost: external_exports.string().optional()
  }).optional()
});
...
function formatStructuredLog(stage, status, requestId, message, extra = {}) {
...
__name(formatStructuredLog, "formatStructuredLog");
var PipelineExecutor = class {
  constructor(stages) {
    this.stages = stages;
  }
  async executeJob(job, env, onUpdate) {
    let currentJob = { ...job };
    for (const stage of this.stages) {
      const stageKey = stage.name.toLowerCase();
      currentJob.status = stage.name;
      ...
      currentJob.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
      onUpdate(currentJob);
      let success = false;
      let attempt = 0;
      let lastError = null;
      while (attempt < stage.maxRetries && !success) {
        ...
        try {
          const result = await Promise.race([
            ...
            new Promise(
              (_, reject) => setTimeout(() => reject(new Error("Stage timeout exceeded")), stage.timeoutMs)
            )
          ]);
          if (result.success) {
            currentJob = {
...
__name(PipelineExecutor, "PipelineExecutor");

// src/domain/pipeline-stage.ts
var PipelineStage = class {
  maxRetries = 3;
  timeoutMs = 3e4;
  async rollback(job, env) {
  }
};
__name(PipelineStage, "PipelineStage");

// src/providers/transcript.ts
var ScraperTranscriptProvider = class {
  async getTranscript(videoId) {
    ...
    try {
      ...
      const transcript = textMatches.map((tag) => {
        return tag.replace(/<[^>]*>/g, "").replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/&amp;#39;/g, "'").trim();
...
__name(ScraperTranscriptProvider, "ScraperTranscriptProvider");
var FallbackTranscriptProvider = class {
...
__name(FallbackTranscriptProvider, "FallbackTranscriptProvider");

// src/adapters/sources.ts
var YouTubeAdapter = class {
  ...
  async extract(url) {
    ...
    try {
      ...
      const match2 = url.match(/(?:youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=)|youtu\.be\/)([^"&?\/\s]{11})/i);
      if (match2)
        videoId = match2[1];
    }
    if (!videoId) {
...
__name(YouTubeAdapter, "YouTubeAdapter");
var WebPageAdapter = class {
  canHandle(url, type) {
  ...
  async extract(url) {
    try {
      ...
      const html = await res.text();
      let cleanText = html.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, "").replace(/<style\b[^<]*(?:(?!<\/style>)<[^<]*)*<\/style>/gi, "").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();
...
__name(WebPageAdapter, "WebPageAdapter");
...
__name(RawTextAdapter, "RawTextAdapter");
...
__name(CloudflareAIProvider, "CloudflareAIProvider");

// src/utils/mdx-builder.ts
var MdxBuilder = class {
  static build(article, videoId, videoUrl) {
...
`;
    yaml += `pubDate: ${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}
`;
...
__name(MdxBuilder, "MdxBuilder");

// src/pipeline/stages.ts
var SourceExtractionStage = class extends PipelineStage {
  name = "Extracting";
...
__name(SourceExtractionStage, "SourceExtractionStage");
...
__name(ResearchStage, "ResearchStage");
var DraftingStage = class extends PipelineStage {
  name = "Drafting";
  description = "Build the final structured Markdown article and content bundles";
  extractAndVerify(rawMarkdown) {
...
${research}`;
    ...
    try {
      const match2 = cleanedMarkdown.match(/^---\r?\n([\s\S]+?)\r?\n---/);
      if (match2) {
        const yamlLines = match2[1].split("\n");
...
__name(DraftingStage, "DraftingStage");
...
__name(PlanningStage, "PlanningStage");
var ParallelDraftingStage = class extends PipelineStage {
  ...
  async execute(job, env) {
    ...
    const draftingPromises = articlesSpec.map(async (spec) => {
...
__name(ParallelDraftingStage, "ParallelDraftingStage");
...
- [ ] Ready to merge`;
  ...
  static async createMultiPullRequest(articles, sourceUrl, env, requestId) {
    const owner = env.GITHUB_OWNER || "jaswanthsai7";
    const repo = env.GITHUB_REPO || "Nadhebe";
    const token = env.GITHUB_TOKEN;
    const baseBranch = env.GITHUB_BRANCH || "master";
    ...
    try {
      ...
      let prBody = `## \u{1F4D6} Multi-Blog Content Cluster: [YouTube Reference](${sourceUrl})

`;
      prBody += `An automated AI content cluster has been generated and committed to this branch.

`;
      prBody += `### \u{1F6E0}\uFE0F Pipeline Details
`;
...
`;
      prBody += `*   **Generation Time**: \`${(/* @__PURE__ */ new Date()).toLocaleString()}\`
`;
      prBody += `*   **Model**: \`@cf/meta/llama-3.2-3b-instruct\`

`;
      prBody += `### \u{1F4C2} Affected Folders
`;
      prBody += `*   ${foldersAffected}

`;
      prBody += `### \u{1F4CA} Generated Articles
`;
...
__name(IssueService, "IssueService");

// src/services/job.service.ts
var JobService = class {
  static async createJob(input, env, requestId) {
  ...
  static async runAutoPipeline(input, env, requestId) {
    const id = crypto.randomUUID();
    const now = (/* @__PURE__ */ new Date()).toISOString();
    const reqId = requestId || "unknown";
    const job = {
...
__publicField(JobService, "jobsMap", /* @__PURE__ */ new Map());
...
var envelope = /* @__PURE__ */ __name((c, success, data, error = null, status = 200) => {
  return c.json({
    success,
    data,
    error,
...
var handleAutoPipeline = /* @__PURE__ */ __name(async (c) => {
  const body = await c.req.json();
  const result = CreateJobSchema.safeParse(body);
  if (!result.success) {
...
// .wrangler/tmp/bundle-hRJn68/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-hRJn68/middleware-loader.entry.ts
