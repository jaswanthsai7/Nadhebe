...
replacement_new = """    <!-- Standalone floating footer pill -->
    <div class="mt-32" data-reveal>
      <div class="w-full flex items-center justify-center py-20 rounded-2xl border border-border/40 dark:border-border-dark/40 bg-surface/85 dark:bg-surface-dark/85 text-body font-sans font-semibold text-ink2 dark:text-ink2-dark select-none shadow-medium backdrop-blur-md hover:shadow-large transition-all duration-300 cursor-default">
        That's all, Love <span class="text-accent dark:text-accent-dark inline-block hover:scale-120 transition-transform duration-150 cursor-default ml-8">🧡</span>
      </div>
    </div>"""
...
for file_path in category_files:
    ...
    if target_old in content:
        ...
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(content)
        print(f"Updated card to full width in {file_path}")
    else:
        # Fallback replacing inline-flex values
        if "inline-flex items-center gap-12 px-32 py-16" in content:
            content = content.replace("inline-flex items-center gap-12 px-32 py-16", "w-full flex items-center justify-center py-20")
            content = content.replace("flex justify-center\" data-reveal>", "data-reveal>")
            with open(file_path, "w", encoding="utf-8") as f:
                f.write(content)
            print(f"Fallback full width updated in {file_path}")
